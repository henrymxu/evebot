...
namespace TrackMessageGenerator {
    export function createSongTrackNowPlayingEmbed(
        track: YoutubeTrack
    ): MessageEmbed {
        const userMentionString = GuildUtils.createUserMentionString(
    ...
    export function createLinkTrackCurrentlyPlayingEmbed(
        track: Track,
        url: string
    ): MessageEmbed {
    ...
    export function createLinkTrackNewlyQueuedEmbed(
        track: Track,
        url: string
    ): MessageEmbed {
...
export namespace TrackMessageFactory {
    ...
    export function createCurrentlyPlayingEmbed(track: Track): MessageEmbed {
        if (track instanceof YoutubeTrack) {
            return TrackMessageGenerator.createLinkTrackCurrentlyPlayingEmbed(
                track,
                track.getYoutubeTrackInfo().url
            )
        }
    ...
    export function createTrackNewlyQueuedEmbed(track: Track): MessageEmbed {
        if (track instanceof YoutubeTrack) {
            return TrackMessageGenerator.createLinkTrackNewlyQueuedEmbed(
                track,
                track.getYoutubeTrackInfo().url
            )
        }
    ...
    export function createQueuedTracksMessage(
        ...
        tracks.forEach((track) => {
            ...
            title =
                track.isPlaying() || track.isPaused() ? `< ${title} > ` : title
            let length = Utils.convertSecondsToTimeString(track.getLength())
            tableData.push([
                title,
                track.getArtist(),
                track.getRequester(context),
                length,
            ])
        ...
        response += `\n# Total Queue Time: ${Utils.convertSecondsToTimeString(
            totalLength
        )}`
        return response
    }
    ...
    function createProgressBar(track: Track): string {
        ...
        const filledLength = Math.floor(
            barLength * (track.getElapsedTimeInSeconds() / track.getLength())
        )
        const numberOfBrackets = filledLength > 2 ? 1 : 0 // Open and close brackets
        const numberOfFilled =
            filledLength > 2 ? filledLength - 2 : filledLength
        const bar =
            '<'.repeat(numberOfBrackets) +
            '='.repeat(numberOfFilled) +
            '>'.repeat(numberOfBrackets) +
            '-'.repeat(barLength - (numberOfFilled + 2 * numberOfBrackets))
        const elapsedTimeString = Utils.convertSecondsToTimeString(
            track.getElapsedTimeInSeconds()
        )
        const totalTimeString = Utils.convertSecondsToTimeString(
            track.getLength()
        )
