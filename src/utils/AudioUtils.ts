...
export namespace AudioUtils {
    ...
    export function convertStereoBufferToMonoBuffer(buffer: Buffer): Buffer {
        ...
        for (let i = 0; i < newBuffer.length / 2; ++i) {
            ...
            const right =
                (buffer[i * 4 + 2 + HI] << 8) | (buffer[i * 4 + 2 + LO] & 0xff)
     ...
     */
    export function createDownSampleTransformStream(
        resultMode: number = 0
    ): Transform {
    ...
    export function convertBufferToMp3Buffer(
        audioBuffer: Buffer,
        title: string,
        author: string
    ): Promise<Buffer> {
    ...
    export function convertMp3StreamToOpusStream(
