...
export namespace Lifecycle {
                ...
                if (hasUserLeftChannel(oldState, newState)) {
                    ...
                    GlobalContext.get(oldState.guild.id).then((context) => {
                        context
                            ...
                            .userLeftChannel(oldState.member.user)
                    })
                }
                    ...
                    if (isItself(client, newState)) {
                        ...
                        GlobalContext.get(newState.guild.id).then((context) => {
                            context
                                .getProvider()
                                .getVoiceConnectionHandler()
                                .joinVoiceChannel(newState.channel)
                        })
                        return
                    }
                    GlobalContext.get(oldState.guild.id).then((context) => {
    ...
    function hasUserChangedChannel(
        oldState: VoiceState,
        newState: VoiceState
    ): boolean {
        return (
