...
export default class Microsoft
    implements SpeechGenerator, SpeechRecognizer, SpeechProvider {
    ...
    asyncGenerateSpeechFromText(
        message: string,
        voice: string = 'en-CA-Linda'
    ): Promise<SpeechGeneratorResult> {
        const speechConfig = SpeechSDK.SpeechConfig.fromSubscription(
            Keys.get(configVars[0]),
        ...
        return new Promise<SpeechGeneratorResult>((res, rej) => {
            synthesizer.speakTextAsync(
                ...
                (result) => {
                    ...
                    const streamLengthInSeconds =
                        result.audioData.byteLength / sampleRate
    ...
    recognizeTextFromSpeech(audioStream: Readable): Promise<string> {
        ...
        let recognizer = new SpeechSDK.SpeechRecognizer(
            speechConfig,
            audioConfig
        )
