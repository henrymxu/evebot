...
export namespace Search {
            ...
            parse(query)
                ...
                .catch((err) => {
                    Logger.e(
                        null,
                        TAG,
                        `Error searching for track ${query}, reason: ${err}`
                    )
                    rej(err)
                })
        })
    ...
    export function searchAlbum(album: Album): Promise<Track[]> {
        ...
        album.tracks.forEach((track) => {
            promises.push(
                new Promise<Track>((res1, rej1) => {
                    parse(`${track.artist} - ${track.name}`).then(
                        (searchResult) => {
                            resolveSingleTrack(searchResult, track).then(
                                (trackResult) => {
                                    res1(trackResult)
                                }
                            )
        ...
        for (let info of result.infos) {
            ...
            if (basicInfo.formats.length > 0) {
                ...
                Logger.d(
                    null,
                    TAG,
                    `Found ${basicInfo.videoDetails.title} for ${result.metadata.query}`
                )
