...
export default class YoutubeTrack extends Track {
    ...
    loadStream(context: GuildContext): Promise<Readable> {
        ...
        const announceResult = sGen
            ? sGen.asyncGenerateSpeechFromText(`Now Playing ${this.getInfo().title}`)
        ...
        return Promise.all([announceResult, songStream])
            .then((streams: (Readable | SpeechGeneratorResult | undefined)[]) => {
                const announceStream = streams[0] ? (streams[0] as SpeechGeneratorResult).stream : undefined;
                this.stream = announceStream
                    ? StreamUtils.mergeAsync(announceStream, streams[1] as Readable)
