...
export default class DJ {
    ...
    constructor(context: GuildContext) {
        ...
        this.radio = new SpotifyRadio(
            context,
            (query: string, requesterId: string, message?: Message) => {
    ...
    request(
        ...
        switch (mode) {
            case QueryMode.Album: {
                playFunc = () => {
                    return this.playAlbum(query, requesterId, message)
                }
                break
            }
            default: {
        ...
        playFunc()
            .catch((err) => {
                Logger.e(
                    this.context,
                    DJ.name,
                    `Error queuing ${query}, reason: ${err}`
                )
            })
            .finally(() => {
    ...
    private onTracksQueued(tracks: Track[]) {
        const message = TrackMessageFactory.createQueuedTracksMessage(
            this.context,
            tracks
        )
        this.context
            ...
            .send({
                content: message,
                id: 'queue',
                message: tracks[0].metaData.source,
