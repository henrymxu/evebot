...
export default class PrivilegesCommand extends Command {
    readonly options: CommandOptions = {
        ...
        arguments: [
            ...
            },
            {
                ...
                flag: 'grant',
                description:
                    'Grant a member / role privilege to use the command',
                required: false,
                type: ArgumentType.STRING,
                array: true,
            },
            {
                ...
                flag: 'deny',
                description:
                    'Deny a member / role privilege to use the command',
                required: false,
                type: ArgumentType.STRING,
                array: true,
            },
        ...
        examples: [
            'privileges play -grant Liam @Olivia Admins -deny @Kevin Newcomers',
        ],
    }
    ...
    execute(
        ...
        if (!args.get('privilegeName')) {
            const response = createPrivilegesListMessage(
                context.getConfig().getPrivileges()
            )
            context
                .getProvider()
                .getResponder()
        ...
        if (args.get('grant') || args.get('deny') || args.get('remove')) {
            context.getConfig().grantEntitiesPrivilege(
                privilegeName,
                (args.get('grant') || []).map((name) =>
                    mapNameToUserOrRole(context, name)
                )
            )
            context.getConfig().denyEntitiesPrivilege(
                privilegeName,
                (args.get('deny') || []).map((name) =>
                    mapNameToUserOrRole(context, name)
                )
            )
            context.getConfig().removeEntitiesFromPrivilege(
                privilegeName,
                (args.get('remove') || []).map((name) =>
                    mapNameToUserOrRole(context, name)
                )
            )
        }
        const response = createPrivilegeMessage(
            context,
            context.getConfig().getPrivilege(privilegeName)
        )
        context
            .getProvider()
            .getResponder()
...
function createPrivilegeMessage(
    context: GuildContext,
    privilege: Privilege
): string {
    if (!privilege) {
    ...
    function addRowsToTable(
        ...
        users.forEach((userID) => {
            rows.push([
                'User',
                GuildUtils.parseUserFromUserID(context, userID).username,
            ])
        })
        roles.forEach((roleID) => {
            rows.push([
                'Role',
                GuildUtils.parseRoleFromRoleID(context, roleID).name,
            ])
        })
        return rows
    }
    let tableData = []
    tableData.push(
        ...addRowsToTable(privilege.grantedUsers, privilege.grantedRoles)
    )
    tableData.push(['Type', '< Deny >'])
    tableData.push(
        ...addRowsToTable(privilege.deniedUsers, privilege.deniedRoles)
    )
