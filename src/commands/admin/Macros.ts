...
export default class MacrosCommand extends Command {
    readonly options: CommandOptions = {
        ...
        examples: [
            'macros closer -c play closer',
            'macros clipThat -c recite @Eve',
        ],
    }
    ...
    execute(
        ...
        if (!args.get('list') && (args.get('create') || args.get('delete'))) {
            if (args.get('create')) {
                context
                    .getConfig()
                    .addMacro(args.get('macro'), {
                        command: args.get('create'),
                        creator: source.id,
                    })
        ...
        context
            ...
            .forEach((command, key) => {
                tableData.push([
                    key,
                    command.command,
                    GuildUtils.parseUserFromUserID(context, command.creator)
                        .username,
                ])
            })
        ...
        context
            ...
            .getResponder()
            .send(
                { content: content, message: message, options: { code: true } },
                20
            )
    }
}
