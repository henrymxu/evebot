...
export default class LoggingCommand extends Command {
        ...
        arguments: [
            {
                ...
                validate: (context, arg) =>
                    GuildUtils.findTextChannelByName(context, arg) != null,
            },
            {
                key: 'level',
                flag: 'l',
                description:
                    'Logging level (i = info, d = debug, w = warning, e = error)',
                required: false,
                type: ArgumentType.STRING,
                validate: (context, arg) => {
    ...
    execute(
        ...
        const textChannel = GuildUtils.findTextChannelByName(
            context,
            args.get('channel')
        )
...
function createCurrentLoggingTextChannelEmbed(
    context: GuildContext
): MessageEmbed {
    const logging = context.getConfig().getLogging()
    const currentTextChannel = GuildUtils.findTextChannelByID(
        context,
        logging.channelID
    )
