...
export default class ClipCommand extends VoiceCommand {
        ...
        arguments: [
            ...
            },
            {
                ...
                validate: (context, arg) =>
                    parseInt(arg) > 0 && parseInt(arg) <= 20,
            },
            {
                key: 'caption',
                flag: 'c',
    ...
    execute(
        ...
        const voiceStream = context
            .getProvider()
            .getVoiceConnectionHandler()
            .getVoiceStreamForUser(user)
        if (voiceStream == null) {
            Logger.w(
                context,
                ClipCommand.name,
                `No audioStream for ${user.tag} [${user.id}]`
            )
            context
                .getProvider()
                .getResponder()
                .error('No listening stream registered for user', message)
            return
        }
        context.getProvider().getResponder().startTyping(message)
        AudioUtils.convertBufferToMp3Buffer(
            voiceStream.getBuffer(args.get('length')),
            args.get('caption'),
            user.tag
        )
            .then((buffer) => {
                const embedMessage = MessageGenerator.createBasicEmbed(
                    `Recording from [${GuildUtils.createUserMentionString(
                        user.id
                    )}]`
                )
                const embed = MessageGenerator.attachFileToEmbed(
                    embedMessage,
                    buffer,
                    `${args.get('caption')}.mp3`
                )
                context
                    .getProvider()
                    .getResponder()
