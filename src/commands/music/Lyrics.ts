    ...
    execute(context: GuildContext, source: User, args: Map<string, any>, message?: Message) {
        ...
        if (!query) {
            context
                .getProvider()
                .getResponder()
                .error(`There is no song playing! Provide a song name for some lyrics`, message)
        }
        GeniusLyrics.get(query, args.get('artist'))
            .then((result) => {
                ...
                context
                    .getProvider()
                    .getResponder()
                    .send({ content: result.lyrics, message: message, options: { code: 'Markdown', embed: embed } })
            })
            .catch((err) => {
