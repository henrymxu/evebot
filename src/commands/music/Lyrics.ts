...
export default class LyricsCommand extends Command {
        ...
        arguments: [
            {
                key: 'query',
                description:
                    'Name of song (if none is provided, currently playing song is used)',
                required: false,
    ...
    execute(
        ...
        const query =
            args.get('query') ||
            context.getProvider().getDJ().getCurrentSong().getTitle()
        if (!query) {
            context
                .getProvider()
                .getResponder()
                .error(
                    `There is no song playing! Provide a song name for some lyrics`,
                    message
                )
        }
        GeniusLyrics.get(query, args.get('artist'))
            .then((result) => {
