...
export class RespectCommand extends Command {
    ...
    execute(
        ...
        context
            .getProvider()
            .getResponder()
            .send({ content: response, id: 'disrespect', message: message }, 30)
    }
}

function respect(context: GuildContext, source: User, target: User): string {
    if (!target) {
        return `here\'s free 69 respects\n ${GuildUtils.createUserMentionString(
            source.id
        )} you have 69(free respects claimable in [right now]!`
    }
    return `${GuildUtils.createUserMentionString(
        source.id
    )} respected ${GuildUtils.createUserMentionString(
        target.id
    )}! (Total respect: 69)`
}
...
function disrespect(context: GuildContext, source: User, target: User): string {
    ...
    return `${GuildUtils.createUserMentionString(
        source.id
    )} just spent 2 respects to disrespect ${GuildUtils.createUserMentionString(
        target.id
    )}! (Remaining respect: -9007199254740991)`
}
