...
export default class StatusCommand extends Command {
    readonly options: CommandOptions = {
        ...
        descriptions: [
            ...
            'Show current memory usage of the bot',
            'Show raw config of the bot',
        ],
        arguments: [],
...
function getStatusResponse(context: GuildContext): string {
    ...
    tableData.push([
        ...
        context.getVoiceConnection()
            ? context.getVoiceConnection().channel.name
            : 'None',
    ])
    ...
    tableData2.push([
        'HotwordEngine',
        context.getVoiceDependencyProvider().getHotwordEngine().getStatus(),
    ])
    tableData2.push([
        'Hotwords',
        context
    ...
    tableData2.push([
        ...
        ((context
            .getVoiceDependencyProvider()
            .getSpeechGenerator() as unknown) as SpeechProvider).getStatus(),
    ])
    tableData2.push([
        'SpeechRecognition',
        ((context
            .getVoiceDependencyProvider()
            .getSpeechRecognizer() as unknown) as SpeechProvider).getStatus(),
    ])
